...
<task type="auto">
  ...
  <files>midl-agent/src/search/bug-report-generator.ts, midl-agent/src/search/bug-report-generator.test.ts, midl-agent/src/search/issue-creator.ts, midl-agent/src/search/issue-creator.test.ts</files>
  <action>
...
**Scope limitation (from deferred ideas):** BugReportGenerator and IssueCreator handle simple fixes only (1-2 line changes, single file). Multi-file refactoring suggestions are OUT OF SCOPE for Phase 3.
...
Create `bug-report-generator.test.ts`:
- Test: extracts error message from description containing "Error: ..."
- Test: extracts SDK version from "@midl/react 1.2.3" pattern
- Test: extracts network from "testnet" / "mainnet" mentions
- Test: generates title under 80 chars
- Test: formatAsMarkdown produces valid markdown with all sections
- Test: formatAsGitHubLink produces valid URL-encoded link
- Test: handles empty/minimal description gracefully

Create `issue-creator.test.ts`:
- Test: createFromDraft returns created=true with issue number/url on success (mock GitHubClient)
- Test: createFromDraft returns created=false with fallbackUrl on permission error
- Test: fallbackUrl is a valid GitHub new issue URL
  </action>
  <verify>
1. TypeScript compiles: `cd midl-agent && npx tsc --noEmit`
2. Run tests: `cd midl-agent && npx vitest run src/search/bug-report-generator.test.ts src/search/issue-creator.test.ts`
  </verify>
  <done>BugReportGenerator extracts context and formats reports. IssueCreator creates GitHub issues with fallback to manual submission link on auth failure. Tests verify extraction logic and fallback behavior.</done>
</task>
...
<verification>
```bash
cd midl-agent && npx tsc --noEmit && npx vitest run src/search/bug-report-generator.test.ts src/search/issue-creator.test.ts
```
TypeScript compiles cleanly. All new classes export properly. Tests verify extraction and fallback logic.
</verification>
