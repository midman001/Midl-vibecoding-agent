...
<task type="auto">
  ...
  <files>midl-agent/src/search/similarity-scorer.ts, midl-agent/src/search/duplicate-detector.ts, midl-agent/src/types/search-types.ts, midl-agent/src/search/constants.ts</files>
  <action>
    ...
    2. Extract shared stop words to midl-agent/src/search/constants.ts:
       - Export `STOP_WORDS: ReadonlySet<string>` containing the stop word list currently in term-extractor.ts
       - Update term-extractor.ts to import from constants.ts instead of defining its own list
       - SimilarityScorer will also import from constants.ts

    3. Create similarity-scorer.ts with SimilarityScorer class:
       ...
       - Algorithm (simple, NOT ML):
         ...
         b. Remove stop words using STOP_WORDS from constants.ts
    ...
    4. Create duplicate-detector.ts with DuplicateDetector class:
    ...
    5. Wire cache into IssueSearcher (modify issue-searcher.ts):
  ...
  <verify>
    1. Run `cd midl-agent && npx tsc --noEmit` -- compiles cleanly.
    2. Write a quick smoke script (or use node -e) that creates an IssueSearcher with a mock client and a SearchCache, calls search() twice with the same terms, and asserts the mock client was only called once. This confirms cache integration prevents redundant API calls.
       Alternatively, verify by inspecting the code: issue-searcher.ts must contain both `cache.get(` and `cache.set(` calls wrapping the GitHub API call in search().
    3. Verify constants.ts exports STOP_WORDS and term-extractor.ts imports from it (grep for the import).
  </verify>
  <done>SimilarityScorer calculates Jaccard + title boost scores. DuplicateDetector orchestrates full pipeline. IssueSearcher uses cache -- calling search() twice with same terms hits API only once. Stop words shared via constants.ts. All TypeScript compiles.</done>
</task>

<task type="auto">
  <name>Task 2: Unit tests for scorer, detector, and cache integration</name>
  <files>midl-agent/src/search/similarity-scorer.test.ts, midl-agent/src/search/duplicate-detector.test.ts, midl-agent/src/search/issue-searcher.test.ts</files>
  <action>
    ...
    3. Create issue-searcher.test.ts (cache integration tests):
       - Create a mock GitHub client and a real SearchCache instance
       - Inject both into IssueSearcher via constructor DI
       - "calls cache.get() before making API call" -- spy on cache.get, call search(), assert cache.get was called
       - "returns cached results on cache hit without calling API" -- prime cache with cache.set(), call search(), assert mock client was NOT called, assert results match cached data
       - "calls API and cache.set() on cache miss" -- call search() with empty cache, assert mock client WAS called, spy on cache.set and assert it was called with the results
       - "second search with same terms uses cache" -- call search() twice with same description, assert mock client called exactly once
  ...
  <done>Scorer tests verify Jaccard algorithm behavior. Detector tests verify threshold logic, sorting, and formatting. Cache integration tests verify cache.get() called before API, cache.set() called after miss, and cached results returned on hit. All tests green.</done>
</task>
...
- IssueSearcher uses cache (second call for same terms does not hit GitHub) -- verified by unit test
- formatResults() produces readable output with [DUPLICATE] prefix and percentages
- Stop words defined once in constants.ts, imported by both term-extractor and similarity-scorer
</verification>
...
- INTEG-05: Cache integrated into IssueSearcher, prevents redundant API calls (tested)
